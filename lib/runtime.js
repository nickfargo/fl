...
(function() {
  var ArrayIterator, ConcatIterator, CycleIterator, DropIterator, EmptyIterator, FilterIterator, InterleaveIterator, IterateIterator, IteratorOutput, MapIterator, MapIterator_variadic, PairwiseArrayIterator, PartitionIterator, RangeIterator, ReductionIterator, RepeatIterator, TakeIterator, assertUnfinished, callable, compare, complement, concat, cycle, decrement, dropWhile, empty, filter, generatorOf, identity, increment, interleave, isEven, isOdd, iterate, iteratorOf, map, multiply, partition, range, reduce, reductions, repeat, slice, sum, takeWhile, throwIteratorFinished, toArray, _map, _map_variadic;
  ...
  map = function(fn, sequence, rest) {
    if (sequence == null) {
      ...
      return _map(fn, sequence);
    } else {
      return _map_variadic(fn, slice.call(arguments, 1));
    }
  };

  _map = function(fn, sequence) {
    return function() {
      return new MapIterator(fn, iteratorOf(sequence));
    };
  };

  _map_variadic = function(fn, sequences) {
    return function() {
