...
(function() {
  var apply, compare, comparisonOperator, complement, compose, concat, cycle, decreasing, decrement, drop, dropWhile, empty, filter, first, generatorOf, identity, increasing, increment, interleave, interpose, isEven, isOdd, iterate, iteratorOf, map, multiply, notDecreasing, notIncreasing, partial, partition, partitionBy, range, reduce, reductions, remove, repeat, rest, slice, sort, splitAt, splitWith, sum, take, takeWhile, toArray, _ref;
  ...
  notIncreasing = comparisonOperator(function(x, y) {
    return 0 <= compare(x, y);
  });

  notDecreasing = comparisonOperator(function(x, y) {
    return 0 >= compare(x, y);
  });
  ...
  exports.signatures = [
    {
      ...
      notIncreasing: notIncreasing,
      notDecreasing: notDecreasing,
      apply: apply
    }, {
