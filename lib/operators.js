...
(function() {
  var apply, comparator, compare, complement, compose, concat, cycle, decreasing, decrement, drop, dropWhile, empty, filter, first, generatorOf, identity, increasing, increment, interleave, interpose, isEven, isOdd, iterate, iteratorOf, map, multiply, nondecreasing, nonincreasing, partial, partition, partitionBy, range, reduce, reductions, remove, repeat, rest, slice, sort, sortBy, splitAt, splitWith, sum, take, takeWhile, toArray, _ref;

  _ref = require('./runtime'), generatorOf = _ref.generatorOf, iteratorOf = _ref.iteratorOf, sum = _ref.sum, multiply = _ref.multiply, compare = _ref.compare, comparator = _ref.comparator, toArray = _ref.toArray, empty = _ref.empty, repeat = _ref.repeat, cycle = _ref.cycle, iterate = _ref.iterate, interleave = _ref.interleave, range = _ref.range, filter = _ref.filter, map = _ref.map, reduce = _ref.reduce, reductions = _ref.reductions, concat = _ref.concat, partition = _ref.partition, partitionBy = _ref.partitionBy, takeWhile = _ref.takeWhile, dropWhile = _ref.dropWhile;
  ...
  increasing = comparator(function(x, y) {
    return 0 > compare(x, y);
  });

  decreasing = comparator(function(x, y) {
    return 0 < compare(x, y);
  });

  nonincreasing = comparator(function(x, y) {
    return 0 <= compare(x, y);
  });

  nondecreasing = comparator(function(x, y) {
    return 0 >= compare(x, y);
  });
  ...
  sortBy = function(keyFn, fn, sequence) {
    if (sequence == null) {
      sequence = fn;
      fn = compare;
    }
    return sort((function(x, y) {
      return fn(keyFn(x), keyFn(y));
    }), sequence);
  };
